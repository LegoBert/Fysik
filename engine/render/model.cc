        ...
        for (auto const& primitive : mesh.primitives)
        {
            ...
            for (auto const& attribute : primitive.attributes)
            {
				...
				if (attribute.first == "POSITION") {
					...
					glGetBufferSubData(GL_ARRAY_BUFFER, attr.offset, numVertices * sizeof(glm::vec3), p.vertices.data());
					...
					model.aabb.max = maxAABB;
					model.aabb.min = minAABB;
				}
            }
			...
			if (p.indexType == GL_UNSIGNED_SHORT) {
				// Assuming 16-bit indices
				std::vector<uint16_t> tempIndices(p.numIndices);
				glGetBufferSubData(GL_ELEMENT_ARRAY_BUFFER, p.offset, p.numIndices * sizeof(uint16_t), tempIndices.data());
				p.indices.assign(tempIndices.begin(), tempIndices.end());
			}
			else if (p.indexType == GL_UNSIGNED_INT) {
				// Assuming 32-bit indices
				glGetBufferSubData(GL_ELEMENT_ARRAY_BUFFER, p.offset, p.numIndices * sizeof(uint32_t), p.indices.data());
			}
...
LoadQuad(glm::vec4 rgba)
{
	...
	ModelId const mid = (ModelId)modelAllocator.size();
	modelAllocator.push_back(mdl);
	modelRegistry.emplace("quad", mid);
	return mid;
}
